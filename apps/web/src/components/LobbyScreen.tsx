...
export function LobbyScreen({ room, client, connection, error }: { readonly room: RoomSnapshot; readonly client: MultiplayerClient; readonly connection: ConnectionState; readonly error: string | null }) {
  ...
  function adjustTimer(key: keyof TurnTimerSettings, seconds: number) {
    setTimerSettings((current) => ({ ...current, [key]: Math.max(15, Math.min(600, current[key] + seconds)) }));
  }
    ...
    <section className="lobby-content">
      ...
      <div className="lobby-grid">
        ...
        <aside className="lobby-sidebar"><form className="profile-editor" onSubmit={saveProfile}><span className="eyebrow">Your profile</span><label><span>Name</span><input value={name} maxLength={24} onChange={(event) => setName(event.target.value)} /></label><fieldset className="color-field"><legend>Playing color</legend><div>{PLAYER_COLOR_OPTIONS.map((option) => <button key={option.id} type="button" disabled={room.members.some((member) => member.id !== viewer.id && member.color === option.id)} className={color === option.id ? "selected" : ""} aria-label={option.label} style={{ "--swatch": option.value } as React.CSSProperties} onClick={() => setColor(option.id)}><i /></button>)}</div></fieldset><button className="profile-save" disabled={name.trim().length < 2 || (name.trim() === viewer.name && color === viewer.color)}>Save profile</button></form>
          ...
          {viewer.isHost && <form className="timer-panel" onSubmit={saveTimers}><span className="eyebrow">Turn timers</span><h3>Table pace</h3><div className="timer-fields">{timerFields.map(([key, label, hint]) => <label key={key}><span><strong>{label}</strong><small>{hint}</small></span><span className="timer-control"><span className="timer-value"><input type="number" min={15} max={600} value={timerSettings[key]} onChange={(event) => setTimerSettings({ ...timerSettings, [key]: Math.max(15, Math.min(600, Number(event.target.value) || 15)) })} /><small>sec</small></span><span className="timer-adjustments" aria-label={`Adjust ${label} timer`}><button type="button" onClick={() => adjustTimer(key, -30)} aria-label={`Decrease ${label} by 30 seconds`}>−30</button><button type="button" onClick={() => adjustTimer(key, -15)} aria-label={`Decrease ${label} by 15 seconds`}>−15</button><button type="button" onClick={() => adjustTimer(key, 15)} aria-label={`Increase ${label} by 15 seconds`}>+15</button><button type="button" onClick={() => adjustTimer(key, 30)} aria-label={`Increase ${label} by 30 seconds`}>+30</button></span></span></label>)}</div><button className="profile-save" type="submit" disabled={JSON.stringify(timerSettings) === JSON.stringify(room.timerSettings)}>Save timing</button></form>}
