...
interface TradeDialogProps {
  ...
  readonly onDomesticOffer?: (partnerId: PlayerId, actorGives: ResourceBundle, partnerGives: ResourceBundle) => void;
...
export function TradeDialog({ state, actorId, onClose, onTrade, onDomesticOffer, counterOffer, onCounter }: TradeDialogProps) {
  ...
  const [partnerId, setPartnerId] = useState(counterOffer ? (counterOffer.actorId === actorId ? counterOffer.partnerId : counterOffer.actorId) : state.playerOrder.find((id) => id !== actorId) ?? actorId);
  ...
  const domesticAllowed = state.phase.kind === "player1-actions";
  ...
  function submit() {
    ...
    else if (onDomesticOffer) onDomesticOffer(partnerId, actorGives, partnerGives);
    else onTrade({ type: "DOMESTIC_TRADE", partnerId, actorGives, partnerGives });
  }
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="dialog trade-dialog" role="dialog" aria-modal="true" aria-labelledby="trade-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {!counterOffer && <div className="dialog-tabs"><button type="button" className={tab === "bank" ? "active" : ""} onClick={() => setTab("bank")}><Icon name="ship" />Bank & ports</button><button type="button" className={tab === "player" ? "active" : ""} disabled={!domesticAllowed} title={domesticAllowed ? undefined : "Player 2 may only trade with the bank"} onClick={() => setTab("player")}><Icon name="hand" />Another player</button></div>}
      {tab === "player" && !counterOffer && <label className="field-label">Trade with<select value={partnerId} onChange={(event) => setPartnerId(event.target.value)}>{state.playerOrder.filter((id) => id !== actorId).map((id) => <option key={id} value={id}>{state.players.get(id)?.name}</option>)}</select></label>}
      {tab === "bank" ? <div className="trade-flow"><div className="trade-side"><span>You give</span><div className="resource-picker">{RESOURCE_TYPES.map((resource) => <button type="button" key={resource} className={give === resource ? "selected" : ""} style={{ "--resource-color": RESOURCE_META[resource].color } as React.CSSProperties} onClick={() => { setGive(resource); if (receive === resource) setReceive(RESOURCE_TYPES.find((item) => item !== resource)!); }}><i />{RESOURCE_META[resource].label}<strong>{rate}</strong></button>)}</div></div><div className="trade-arrow"><Icon name="trade" /><span>{rate}:1 rate</span></div><div className="trade-side"><span>You receive</span><div className="resource-picker">{validResources.map((resource) => <button type="button" key={resource} className={receive === resource ? "selected" : ""} style={{ "--resource-color": RESOURCE_META[resource].color } as React.CSSProperties} onClick={() => setReceive(resource)}><i />{RESOURCE_META[resource].label}<strong>1</strong></button>)}</div></div></div>
      ...
      <div className="dialog-footer"><p>{canTrade ? (tab === "player" ? "Only the cards you offer are checked now; your partner decides whether to accept." : "This trade is available.") : tab === "player" ? "Add cards to both sides and make sure you own everything offered." : "The selected cards are not currently available."}</p><button type="button" className="confirm-button" disabled={!canTrade} onClick={submit}>{counterOffer ? "Send counteroffer" : tab === "player" && onDomesticOffer ? "Send offer" : "Confirm trade"} <Icon name="chevron" /></button></div>
    </section>
  </div>;
}
